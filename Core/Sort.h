...
template <class I, class Less, class Iter_Swap>
void FinalSort__IterSwap(I begin, I end, const Less& less, const Iter_Swap& iter_swap)
{
	if(begin == end)
		return;
	I last = end;
...
template <class I, class Less, class Iter_Swap>
force_inline
void OrderIter2__IterSwap(I a, I b, const Less& less, const Iter_Swap& iter_swap)
{
	if(less(*b, *a))
		iter_swap(a, b);
}
...
template <class I, class Less, class Iter_Swap>
void Sort__IterSwap(I l, I h, const Less& less, const Iter_Swap& iter_swap)
{
	for(;;) {
		int count = int(h - l);
		if(count < 2)
			return;
		if(count < 8) {                         // Final optimized SelectSort
			FinalSort__IterSwap(l, h, less, iter_swap);
			return;
		}
		int pass = 4;
		for(;;) {
			I middle = l + (count >> 1);        // get the middle element
			OrderIter2__IterSwap(l, middle, less, iter_swap);      // sort l, middle, h-1 to find median of 3
			OrderIter2__IterSwap(middle, h - 1, less, iter_swap);
			OrderIter2__IterSwap(l, middle, less, iter_swap);      // median is now in middle
			iter_swap(l + 1, middle);            // move median pivot to l + 1
			I ii = l + 1;
			for(I i = l + 2; i != h - 1; ++i)   // do partitioning; already l <= pivot <= h - 1
				if(less(*i, *(l + 1)))
					iter_swap(++ii, i);
			iter_swap(ii, l + 1);                // put pivot back in between partitions
			I iih = ii;
			while(iih + 1 != h && !less(*ii, *(iih + 1))) // Find middle range of elements equal to pivot
				++iih;
			if(pass > 5 || min(ii - l, h - iih) > (max(ii - l, h - iih) >> pass)) { // partition sizes ok or we have done max attempts
				if(ii - l < h - iih - 1) {       // recurse on smaller partition, tail on larger
					Sort__IterSwap(l, ii, less, iter_swap);
					l = iih + 1;
				}
				else {
					Sort__IterSwap(iih + 1, h, less, iter_swap);
					h = ii;
				}
				break;
			}
			iter_swap(l, l + (int)Random(count));     // try some other random elements for median pivot
			iter_swap(middle, l + (int)Random(count));
			iter_swap(h - 1, l + (int)Random(count));
			pass++;
		}
	}
}
